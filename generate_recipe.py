...
def generate_recipe(image: Image.Image, user_goals: str, model, processor):
    ...
    inputs = processor(text=prompt_ingredients, images=image, return_tensors="pt").to(
        device, model.dtype
    )
    ...
    identified_ingredients = (
        ingredients_response.split("<|im_start|> assistant")[-1]
        .split("<|im_end|>")[0]
        .strip()
    )
    ...
    recipe_prompt = recipe_generation_prompt.format(
        ingredients=identified_ingredients, user_goals=user_goals
    )
    conversation.append(
        {"role": "user", "content": [{"type": "text", "text": recipe_prompt}]}
    )

    final_prompt = processor.apply_chat_template(
        conversation, add_generation_prompt=True
    )
    inputs_final = processor(text=final_prompt, images=image, return_tensors="pt").to(
        device, model.dtype
    )
    ...
    recipe_text = (
        final_response_full.split("<|im_start|> assistant")[-1]
        .split("<|im_end|>")[0]
        .strip()
    )
